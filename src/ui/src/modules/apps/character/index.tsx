...
const Character = ({ visible, characters, setVisible, setCharacters }) => {
    ...
    const validate: any = (fieldValues = newCharData) => {
        ...
        if ("fn" in fieldValues) temp.fn = fieldValues.fn ? "" : "This field is required.";
        if ("ln" in fieldValues) temp.ln = fieldValues.ln ? "" : "This field is required.";
        if ("gender" in fieldValues) temp.gender = fieldValues.gender ? "" : "This field is required.";
        if ("dob" in fieldValues) temp.dob = fieldValues.dob ? "" : "This field is required.";
    ...
    const formIsValid = (fieldValues = newCharData) => {
        const isValid = fieldValues.fn && fieldValues.ln && fieldValues.gender && fieldValues.dob && Object.values(errors).every((x) => x === "");
            ...
            <Box sx={{ width: "100vw", height: "100vh" }} className="UI_CHARACTER_CONTAINER">
                ...
                <Slide in={state === "start"} direction="left">
                    <Box className="UI_CHARACTER_BUTTON">
                        <Button onClick={() => setState("character")} sx={{ fontSize: "26px", padding: "16px 32px" }}>
                            Play
                        </Button>
                    </Box>
                </Slide>
                <Slide in={state != "start"} direction="up">
                    <Box className="UI_CHARACTER_CHARACTERS">
                        {characters.map((character: any, index: any) => (
                            <div key={index} className="CHARACTER">
                                <img src={character.mugshot || mugshot} alt="" className="CHARACTER_MUGSHOT" />
                                ...
                                <table className="CHARACTER_INFO">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>Net Worth: {currencyFormatter(getCharacterNet(character))}</td>
                                        </tr>
                                    </tbody>
                                </table>
                                <div className="CHARACTER_BUTTONS">
                                    <Button onClick={() => handleOpen(character)} sx={{ fontSize: "16px", padding: "0px 24px" }}>
                                        Delete
                                    </Button>
                                    <Button onClick={() => selectCharacter(character)} sx={{ fontSize: "16px", padding: "0px 24px" }}>
                                        Play
                                    </Button>
                                </div>
                            </div>
                ...
                <Dialog open={open} onClose={() => setOpen(false)} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                    ...
                    <DialogContent>
                        <DialogContentText id="alert-dialog-description">
                            Are you sure you wish to delete {selectedChar?.fn.toUpperCase()} {selectedChar?.ln.toUpperCase()}? This action cannot be undone.
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions>
                ...
                <Dialog open={open2} onClose={() => setOpen2(false)} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                        ...
                        <form>
                            <div style={{ display: "flex", columnGap: "15px" }}>
                                <TextField
                                    ...
                                    autoComplete="none"
                                    {...(errors["fn"] && { error: true, helperText: errors["fn"] })}
                                    fullWidth
                                    variant="standard"
                                />
                                <TextField
...
const mapDispatchToProps = (dispatch: any) => ({
    setVisible: (visible: boolean) => dispatch({ type: "SET_CHARACTER_STORE_VISIBILITY", visible }),
    setCharacters: (characters: boolean) => dispatch({ type: "SET_CHARACTER_STORE_CHARACTERS", characters }),
});
