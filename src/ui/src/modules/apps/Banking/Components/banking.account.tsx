...
import { cAccount, setDialogData } from "../banking.store";
...
const account = ({ setState }) => {
  ...
  function updateModelState(model: string, status: boolean) {
    if (model === "newUserModel") dispatch(setDialogData({ var: "userModel", data: status }));
      ...
      <div className={Styles.Content}>
        ...
        <div className={Styles.Cards}>
          <div className={Styles.Title}>
            ...
            Cards
            <IconButton className={Styles.right} aria-label="add">
              <AddIcon />
            </IconButton>
          </div>
            ...
            {CAccount.cards.map((card: any, i) => (
              <div key={i} className={Styles.item}>
                <div className={Styles.label}>
                  ...
                  {`${card.ccn.slice(0, 4)}-${card.ccn.slice(4, 8)}-${card.ccn.slice(8, 12)}-${card.ccn.slice(12, 16)}`} -<div>{card.type.toUpperCase()}</div>
                </div>
                <div className={Styles.amount}>{card.cvc}</div>
                <div className={Styles.id}>{card.name}</div>
                <div className={Styles.date}>
                  {new Date(
                    new Date().setFullYear(new Date(card.exp).getFullYear() + 4)
                  ).toLocaleDateString()}
                </div>
              </div>
            ))}
        ...
        <div className={Styles.AUsers}>
          <div className={Styles.Title}>
            ...
            Authorized Users
            <IconButton onClick={() => updateModelState("newUserModel", true)} className={Styles.right} aria-label="add">
              <AddIcon />
            </IconButton>
          </div>
          <div className={Styles.Wrapper}>
            ...
            {CAccount.authorizedUsers.length <= 0 && (
              ...
              >
                No Authorized Users? Please contact support as soon as possible. [account:{CAccount.accountNumber}]
              </div>
            )}
          </div>
        </div>
